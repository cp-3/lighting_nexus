...
public class PDFUtil {
	...
	//PDF文档访问地址
	private final static String PDF_URL = PropertiesConfigUtil.getProperty("pdf.url");
	//PDF文档存放地址
	private final static String pdf_document_dir = PropertiesConfigUtil.getProperty("pdf.document.dir");
//	private final static String pdf_document_dir = "E:\\Program Files\\apache-tomcat-7.0.57\\webapps\\lighting\\pdf\\";
	//image文件存放地址
	private final static String image_file_dir = PropertiesConfigUtil.getProperty("image.file.dir");
//	private final static String image_file_dir = "E:\\Program Files\\apache-tomcat-7.0.57\\webapps\\lighting\\image\\";
	 ...
	 * @param pdfId
	 */
	public static void buildPDF_1(String pdfId) {
	...
	/**
	 * @生产PDF文件
	 * @param CollectProductInfoDTO collectInfo
	 * @return void
	 */
	public static String buildPDF(CollectProductInfoDTO collectInfo, BrandInfoDTO brandInfo) {
		// 表头
		 String[] tableHead = { "项目名称\nProject Name", collectInfo.getProjectName(), "项目编号\nProject No.", collectInfo.getProjectName(), "区域\nArea", "建筑外立面", "日期\nDate", DateUtils.getStringDate(new Date(),"yyyy/MM/dd") };
		
		logger.info("开始创建PDF文档，文档存放位置：" + pdf_document_dir);
		logger.info("image文件存放位置：" + image_file_dir);
		String PDF_FILE = "";
		if(!StringUtils.isEmpty(collectInfo.getCollectId()))
			PDF_FILE = collectInfo.getCollectId()+".pdf";
		else
			PDF_FILE = collectInfo.getProductId()+".pdf";
		
		// 创建文Pdf文挡50, 50, 50,50左右上下距离
		Document document = new Document(PageSize.A4);
		try {
			// 使用PDFWriter进行写文件操作
			PdfWriter.getInstance(document, new FileOutputStream(pdf_document_dir+PDF_FILE));
			document.open();
			// 中文字体
			BaseFont bfChinese = BaseFont.createFont("STSong-Light", "UniGB-UCS2-H", BaseFont.NOT_EMBEDDED);
			Font fontChinese = new Font(bfChinese, 7, Font.BOLD);
			...
			// 添加表头元素
			for (int i = 0; i < colNumber; i++) {
				datatable1.addCell(new Paragraph(tableHead[i], fontChinese8));
			}
			...
			// 添加子元素
			datatable2.addCell(new Paragraph("灯具编号\nCode", fontChinese8));
			PdfPCell cell11 = new PdfPCell(new Paragraph(collectInfo.getModelType(), fontChinese8));
			cell11.setColspan(2);
			datatable2.addCell(cell11);
			datatable2.addCell(new Paragraph("灯具名称\nDescription", fontChinese8));
			PdfPCell cell12 = new PdfPCell(new Paragraph(collectInfo.getProductName(), fontChinese8));
			cell12.setColspan(2);
			datatable2.addCell(cell12);
			datatable2.completeRow();
			datatable2.addCell(new Paragraph("光源\nLight Source", fontChinese8));
			PdfPCell cell21 = new PdfPCell(new Paragraph(MapUtils.getlightSouType(collectInfo.getLightSouType()), fontChinese8));
			cell21.setColspan(2);
			datatable2.addCell(cell21);
			datatable2.addCell(new Paragraph("品牌型号\nBrand Name/Ref. No.", fontChinese8));
			PdfPCell cell22 = new PdfPCell(new Paragraph(collectInfo.getModelType(), fontChinese8));
			cell22.setColspan(2);
			datatable2.addCell(cell22);
			datatable2.completeRow();
			datatable2.addCell(new Paragraph("灯体\nHousing", fontChinese8));
			PdfPCell cell31 = new PdfPCell(new Paragraph("", fontChinese8));
			cell31.setColspan(2);
			datatable2.addCell(cell31);
			datatable2.addCell(new Paragraph("反射器\nReflector", fontChinese8));
			PdfPCell cell32 = new PdfPCell(new Paragraph("", fontChinese8));
			cell32.setColspan(2);
			datatable2.addCell(cell32);
			datatable2.completeRow();
			datatable2.addCell(new Paragraph("饰面\nFinish", fontChinese8));
			PdfPCell cell41 = new PdfPCell(new Paragraph("", fontChinese8));
			cell41.setColspan(2);
			datatable2.addCell(cell41);
			datatable2.addCell(new Paragraph("配件\nAccessories", fontChinese8));
			PdfPCell cell42 = new PdfPCell(new Paragraph(MapUtils.getInstallType(collectInfo.getInstallType()), fontChinese8));
			cell42.setColspan(2);
			datatable2.addCell(cell42);
			datatable2.completeRow();
			datatable2.addCell(new Paragraph("防护等级\nIP", fontChinese8));
			PdfPCell cell51 = new PdfPCell(new Paragraph("", fontChinese8));
			cell51.setColspan(2);
			datatable2.addCell(cell51);
			datatable2.addCell(new Paragraph("变压器\nTransformer", fontChinese8));
			PdfPCell cell52 = new PdfPCell(new Paragraph("", fontChinese8));
			cell52.setColspan(2);
			datatable2.addCell(cell52);
			datatable2.completeRow();
			datatable2.addCell(new Paragraph("尺寸\nDimensions", fontChinese8));
			datatable2.addCell(new Paragraph(collectInfo.getSize(), fontChinese8));
			datatable2.addCell(new Paragraph("宽度Width\nmm", fontChinese8));
			datatable2.addCell(new Paragraph("高度Height\n206mm", fontChinese8));
			datatable2.addCell(new Paragraph("直径Diameter Ø\n180mm Ø", fontChinese8));
			datatable2.addCell(new Paragraph("长度Length\nmm", fontChinese8));
			datatable2.completeRow();
			datatable2.addCell(new Paragraph("厂商\nSupplier", fontChinese8));
			PdfPCell cell61 = new PdfPCell(new Paragraph(brandInfo.getBrandName(), fontChinese8));
			cell61.setColspan(2);
			datatable2.addCell(cell61);
			datatable2.addCell(new Paragraph("联系电话 TEL\n传真  FAX", fontChinese8));
			PdfPCell cell62 = new PdfPCell(new Paragraph(brandInfo.getTelephone()+"\n" +brandInfo.getFax(), fontChinese8 ));
			cell62.setColspan(2);
			datatable2.addCell(cell62);
			datatable2.completeRow();
			datatable2.addCell(new Paragraph("联系人\nContact", fontChinese8));
			PdfPCell cell71 = new PdfPCell(new Paragraph(brandInfo.getLinkMan(), fontChinese8));
			cell71.setColspan(2);
			datatable2.addCell(cell71);
			datatable2.addCell(new Paragraph("地址\nAddress", fontChinese8));
			PdfPCell cell72 = new PdfPCell(new Paragraph(brandInfo.getAddress(), fontChinese8));
			cell72.setColspan(2);
			datatable2.addCell(cell72);
			datatable2.completeRow();
			datatable2.addCell(new Paragraph("备注\nNote", fontChinese8));
			PdfPCell cell81 = new PdfPCell(new Paragraph(collectInfo.getRemark(), fontChinese8));
			cell81.setColspan(5);
			datatable2.addCell(cell81);
			datatable2.completeRow();
